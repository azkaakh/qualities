 ...
 */
 class Comm extends CI_Controller{
    ...
    public function tambahcomm($hasil = null){
        if($this->session->userdata('hak_akses') == 1 && $this->session->userdata('pilihan_project') != 0) {
            $this->load->model('Comodel');
            $data['id'] = $this->Comodel->ambilmaxid()+1;
            $data['hasil'] = $hasil;
            $this->load->view('administrator/comm/tambahcomm', $data);
        }
        else{
            redirect(base_url() . 'login');    
        }
    ...
    public function lihatcomm($hasil = null){
        if($this->session->userdata('hak_akses') == 1 && $this->session->userdata('pilihan_project') != 0) {
            /*$this->load->model('Idenmodel');
            $data['id'] = $this->Idenmodel->ambilmaxid()+1;*/
            $data['hasil'] = $hasil;
            $this->load->view('administrator/comm/lihatcomm', $data);
        }
        else{
            redirect(base_url() . 'login');    
        }
    ...
    public function suntingcomm($hasil = null){
        if($this->session->userdata('hak_akses') == 1 && $this->session->userdata('pilihan_project') != 0) {  
            ...
            $data['idproj'] = $idproj;            
            $data['hasil'] = $hasil;
            $this->load->view('administrator/comm/suntingcomm', $data);
        }
        else{
            redirect(base_url() . 'login');    
        }
    ...
    //================================= Submit Identifikasi Material =======================================================
     ...
     public function submitcomm($hasil = null){
        ...
        if($this->Comodel->tambahisicom($idproj, $idcom, $bagian, $namauji, $idkomp, $qcinspec, $qacoor, $classsur, $ownersur, $tanggalperiksa, $status, $reinspek, $rekomendasi, $path)){
            if ($this->Comodel->tambahitemcomm($idcom, $itemlist)) {
                $config['upload_path'] = './upload/comm';
                ...
                else
                {
                    ...
                    /*$hasil = 'sukses';
                    redirect('comm/tambahcomm/'. $hasil);*/
                    ...
                    $pdf->Cell(26, 0.4, 'Commissioning', 0, 1, "L");
    ...
    public function updateisicomm($hasil = null){
        ...
        if($this->Comodel->suntingisicom($idproj, $idcom, $namauji, $idkomp, $qcinspec, $qacoor, $classsur, $ownersur, $tanggalperiksa, $status, $reinspek, $rekomendasi, $path)){
            if ($this->Comodel->suntingitemcom($idcom, $itemlist)) {
                $config['upload_path'] = './upload/comm';
                $config['allowed_types'] = 'gif|jpg|png';
                $config['max_size'] = '2048000';
     ...
     public function cetakcomm(){
        ...
        foreach ($query->result() as $row)
        {
            ...
            $bagian = $row->bagian_comm;
            $namauji = $row->pengujian_comm;            
            $idkomp = $row->id_kompom; 

            $qcinspec = $row->qc_inspecom;
            $qacoor = $row->qa_coorom;
            $classsur = $row->class_surom;
            $ownersur = $row->owner_surom;
            $tanggalperiksa = $row->tgl_periksaom;
            $status = $row->status_comm;
            $reinspek = $row->tgl_reinspeksiom;
            $rekomendasi = $row->rekomendasi_comm;
            
        }
        
        $querys = $this->Comodel->ambilitemcomm($idenmat);
        ...
        {
            $pdf->Cell($w[0],1,$rows->nama_itemom,'LR',0,'L');
            $pdf->Cell($w[1],1,$rows->isi_itemom,'LR',0,'L');
            $pdf->Cell($w[2],1,$rows->standard_itemom,'LR',0,'L');            
            $pdf->Ln();
        }
        // Closing line
        $pdf->Cell(array_sum($w),0,'','T');
